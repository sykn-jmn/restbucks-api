...
import com.orangeandbronze.restbucks.user.User;
import com.orangeandbronze.restbucks.user.UserRepository;
...
class LoadDatabase {
    ...
    @Bean
    CommandLineRunner initDatabase(DrinkRepository drinkRepository, OrderRepository orderRepository, UserRepository userRepository, BCryptPasswordEncoder passwordEncoder) {
        return args -> {
            ...
            userRepository.save(new User("jeman",passwordEncoder.encode("mama")));
            userRepository.save(new User("eurese",passwordEncoder.encode("bustamante")));
            userRepository.save(new User("viver",passwordEncoder.encode("bungag")));
        };
    }
}